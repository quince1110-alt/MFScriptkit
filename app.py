...
with st.sidebar:
    ...
    st.markdown("---")
    st.info("💡 **核心逻辑**：基于【实见·Sissy】IP，打造“高认知觉醒+打破思维惯性”的爆款内容。")
    st.markdown("#### 使用步骤：")
    st.markdown("1. 输入行业资料/概念/灵感")
    st.markdown("2. 生成3个反常识选题")
    st.markdown("3. 选定标题，生成逐字稿")

# --- 3. 核心逻辑：System Prompt (植入Sissy灵魂) ---
# 这里定义了AI的大脑，包含了你要求的“意识对抗潜意识”、“肉身解题”等逻辑
SYSTEM_PROMPT = """
**Role**: 你是由“实见”品牌打造的【Sissy IP·正念爆款内容策划专家】。
...
st.title("🧘‍♀️ 实见 · Sissy 内容策划机")
st.caption("“痛苦源于认知的错位，解药在于当下的行动。”")

# 检查 API Key
if not api_key:
    st.warning("👈 请先在左侧侧边栏输入您的 Gemini API Key 才能开始使用。")
    st.stop()
...
# 初始化 Session State (用来记住生成的选题，防止刷新消失)
...
user_input = st.text_area(
    ...
    height=150,
    placeholder="例如：关于‘报复性熬夜’的心理学解释，或者‘为什么越休息越累’的概念..."
)

# 生成选题按钮
if st.button("✨ 第一步：生成Sissy风格选题", type="primary"):
    if not user_input:
        st.error("请先输入内容！")
    else:
        with st.spinner("Sissy 正在洞察人性，提炼观点..."):
            try:
                # 发送指令生成选题
                prompt = f"用户提供的素材资料如下：\n{user_input}\n\n请基于Sissy的【真诚流IP】逻辑，生成3个直击痛点的爆款选题，并简述每个选题背后的心理学逻辑。"
...
if st.session_state.topics_text:
    ...
    selected_topic = st.text_input(
        "请复制你最满意的一个标题粘贴在这里：",
        placeholder="例如：《报复性熬夜：我在用牺牲睡眠的方式，夺回生活的控制权》"
    )
